    ...
    stages {               
      stage('Build Sphinx with Docker') {
        steps {
           ...
           sh 'docker run --rm -v $(pwd)/source/suite:/docs/source/suite sphinx_builder ls -latr source;chown -R agent:agent source/suite;ls -latr source/suite;python -m sphinx source/suite build/suite'
                          }
               }
